...
const INITIAL_HOLIDAYS = [
  ...
  { id: 2, name: "Tet Holiday (Lunar New Year)", date: "2026-02-17", type: "Public" },
  { id: 3, name: "Hung Kings' Temple Festival", date: "2026-04-26", type: "Public" },
  { id: 4, name: "Reunification Day", date: "2026-04-30", type: "Public" },
  { id: 5, name: "International Labor Day", date: "2026-05-01", type: "Public" },
  { id: 6, name: "Company Anniversary", date: "2026-06-15", type: "Company" },
  { id: 7, name: "National Day", date: "2026-09-02", type: "Public" },
  { id: 8, name: "Year-End Wellness Day", date: "2026-12-24", type: "Optional" },
];
...
function Holidays() {
  ...
  const handleSaveHoliday = (holiday) => {
    if (holiday.id) {
      setHolidays((prev) =>
        prev.map((h) => (h.id === holiday.id ? { ...h, ...holiday } : h)),
      );
    } else {
      setHolidays((prev) => [
        ...prev,
  ...
  const confirmEditDate = (id) => {
    if (!draftDate) {
      setEditingDateId(null);
      return;
    }
    setHolidays((prev) =>
      prev.map((h) => (h.id === id ? { ...h, date: draftDate } : h)),
    );
    setEditingDateId(null);
  };
  ...
  return (
    <div>
      <div className="toolbar" style={{ marginBottom: "var(--sp-5)" }}>
        <h2 style={{ flex: 1 }}>Holidays 2026</h2>
        <button
