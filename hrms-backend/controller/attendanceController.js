...
const attendanceController = {
  getAll: async (req, res) => {
    try {
      ...
      res.json({ success: true, totalItems, totalPages, currentPage: +pageNumber, items: items.map(attendanceToClient) });
    } catch (error) {
  ...
  checkIn: async (req, res) => {
    try {
      ...
      await logAction(req, {
        ...
        label:      `${record.employee?.name ?? employeeId} on ${date}`,
      });
  ...
  update: async (req, res) => {
    try {
      ...
      await logAction(req, {
        ...
        resource:   "attendance",
        resourceId: record._id,
        label:      `${record.employee?.name ?? req.params.id} on ${attendanceToClient(record).date}`,
      });
